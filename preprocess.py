...
def generate_expansion_and_balance(factor):
    ...
    for b in u_batch:
        ...
        diffFactor = max(num_neg, num_pos)/min(num_neg, num_pos)
        
        if num_pos < num_neg:
            print("Expanding positive cases." )   
            for i in range(int(factor*diffFactor*num_pos)):  
                print(i, "out of", int((factor*diffFactor*num_pos)-num_pos))
                file1, file2 = random.sample(list(lf_pos), 2)
                ############### random.uniform(0,1) OR random.random()
                dataRecombination(file1, file2, perc1=random.uniform(0,1), outputDir=fold+"/data/ProcessData/ST1_umap_train/")
            print("Expanding negative cases." )   
            for i in range(int(factor*num_neg)):  
                print(i, "out of", int((factor*num_neg)-num_neg))
                file1, file2 = random.sample(list(lf_neg), 2)
                ############### random.uniform(0,1) OR random.random()
                dataRecombination(file1, file2, perc1=random.uniform(0,1), outputDir=fold+"/data/ProcessData/ST1_umap_train/")
            ...
            print("Expanding positive cases." )   
            for i in range(int(factor*num_pos)):  
                print(i, "out of", int((factor*num_pos)-num_pos))
                file1, file2 = random.sample(list(lf_pos), 2)
                ############### random.uniform(0,1) OR random.random()
                dataRecombination(file1, file2, perc1=random.uniform(0,1), outputDir=fold+"/data/ProcessData/ST1_umap_train/")
            print("Expanding negative cases." )   
            for i in range(int(factor*diffFactor*num_neg)):  
                print(i, "out of", int((factor*diffFactor*num_neg)-num_neg))
                file1, file2 = random.sample(list(lf_neg), 2)
                ############### random.uniform(0,1) OR random.random()
                dataRecombination(file1, file2, perc1=random.uniform(0,1), outputDir=fold+"/data/ProcessData/ST1_umap_train/")

        elif num_pos == num_neg:
            print("Equal number of positive and control cases, expansion will be performed equally on both outcome samples.")
            print("Expanding positive cases." )   
            for i in range(int(factor*num_pos)):  
                print(i, "out of", int((factor*num_pos)-num_pos))
                file1, file2 = random.sample(list(lf_pos), 2)
