...
def get_hyper_RF(x,y,seed=0,n_jobs = 15): 
        print("hyper RF")            
       # res_max_depth = [2,3,4,5,6,7]
        res_max_features = ["sqrt","log2"]
        #res_max_features = ["sqrt"]
        ...
        def calcu(v):
            ...
            rf = RandomForestClassifier(n_estimators=1001,max_features=max_features,max_depth=max_depth,random_state=seed,class_weight="balanced_subsample",n_jobs=n_jobs,verbose=1)
            rf.fit(x_train, y_train)
            y_pred = rf.predict(x_test)
            return balanced_accuracy_score(y_true=y_test,y_pred=y_pred)
        print("start")
