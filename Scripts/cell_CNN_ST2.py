...
data.load(fold +"/data/ST2/ST2_base_test")
data.save(fold +"/data/ST2/ST2_val1_1")
...
train_data, val_data = data.get_dataload(fold_train=fold +"/data/ST2/ST2_train_1",fold_test=fold +"/data/ST2/ST2_test_1")
...
class Model_Cell_Paper(torch.nn.Module):
    def __init__(self,imput_size, num_markers):
        super().__init__()
        self.flatten = torch.flatten
...
class Neural:
    ...
    def trainning(self,num_epochs,file_out,test_dataset=None,fixed_train_size=False):
...
model = Model_Cell_Paper(imput_size, num_markers=30)
optimizer = optim.Adam(model.parameters(), lr=lr)

net = Neural(train_data,val_data,model=model, loss_f=loss_f,optimizer=optimizer,device=device,sumary_lab="Cell_Paper16bs_26_07_00001",bach_size=batch_size,fixed_train_size=fixed_train_size)                  
net.trainning(num_epochs=200, file_out=fold+"/data/Results/ST2/Cell_Paper16bs_26_07_00001.dat", test_dataset=None)
