...
class Neural:
    ...
    def trainning(self,num_epochs,file_out,test_dataset=None):
        ...
        for epoch in range(num_epochs):
            if(epoch%20==0):
                self._save(fold+"/ST1/ST1_models/"+self.sumary_lab +".dat", epoch, num_epochs)
            print(epoch)
...
optimizer=torch.optim.Adam(model.parameters(), lr=lr)
net = Neural(train_data,val_data,model=model, loss_f=loss_f,optimizer=optimizer,device=device,sumary_lab="modelCVRobust_1307_11_30",bach_size=batch_size)                  
net.trainning(num_epochs=1, file_out=fold+"/Results/ST1/CVRobust/1307_11_30", test_dataset=None)  
